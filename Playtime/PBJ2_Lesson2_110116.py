...
while pb>=1 and jelly>=1 and bread>=2:
    print "Making sandwich #{0}.". format(sandwich)
    bread = bread -2
    sandwich =sandwich+1
    if bread ==0:
        print "All done, you only had enough bread for {0} sandwiches". format(sandwich-1)  
...
while bread >=2 and pb >=1 and jelly >=1:
    print "Making sandwich #{0}.". format(sandwich)
    print "I have enough bread for {0} more sandwiches, enough pb for {1} more, and enough jelly for {2} more.". format(bread/2, pb-1, jelly-1)
    bread = bread -2
    sandwich = sandwich +1
    pb = pb -1
    jelly = jelly-1
    if bread ==0 or pb ==0 or jelly ==0:
